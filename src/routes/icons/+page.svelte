<script>
  ...
  const contentClass = 'rounded-lg dark:bg-neutral-700 mt-4';
  let searchTerm = '';
...
<div class="xl:w-1/3 lg:w-2/5 md:w-1/2 sm:w-3/4 w-full p-4">
  ...
  <Tabs style="pill" {contentClass} class="p-4">
    <TabItem open>
      <span slot="title" class="text-lg">Icon</span>
      <div class="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 px-4 dark:text-white">
        {#each filteredEntries as [name, component]}
          <div class="flex gap-4 items-center text-lg">
            <svelte:component this={component} variation="icon"  class="shrink-0" bind:size={size} />
            {name}
          </div>
        {/each}
      </div>
    ...
    <TabItem>
      <span slot="title" class="text-lg">Random Black</span>
      <div class="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 px-4 dark:text-white">
        {#each filteredEntries as [name, component]}
          <div class="flex gap-4 items-center text-lg">
            <svelte:component
    ...
    <TabItem>
      <span slot="title" class="text-lg">Random White</span>
      <div class="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 px-4 text-white bg-neutral-700">
        {#each filteredEntries as [name, component]}
          <div class="flex gap-4 items-center text-lg ">
